...
[Leetcode官方解法](https://leetcode.com/problems/longest-palindromic-substring/solutions/3598120/longest-palindromic-substring/)
...
1. `StringBuilder result` 需要初始化为一个空对象，或者可以使用 `String` 直接保存最长回文子串。
2. 在更新 `result` 时，你应该使用 `str.substring(i, j + 1)`，而不是 `str.substring(i, j - i + 1)`。
3. 在更新 `result` 时，应该在循环外声明并保存最长的回文子串，而不是每次都更新。
...
- 使用 `start` 和 `end` 变量来追踪最长回文子串的起始和结束索引。
- 使用 `dp[i + 1][j - 1]` 来判断当前的子串是否是回文，并在条件成立时更新 `start` 和 `end`。
- 最后返回 `s.substring(start, end + 1)` 获取正确的回文子串。
...
那么dp[i](#)一定是取最大的，即：`dp[i][j] = max(dp[i + 1][j], dp[i][j - 1];`
